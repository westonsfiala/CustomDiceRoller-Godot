...
static func load_from_save_dict_array(save_dict_array: Array) -> Array[DiePropertyPair]:
	...
	for save_state : Dictionary in save_dict_array:
		var loaded_die_prop_pair : DiePropertyPair = load_from_save_dict(save_state)
...
static func load_from_save_dict(save_state: Dictionary) -> DiePropertyPair:
	...
	var loaded_die : AbstractDie = DieLoaderFactory.load_from_save_dict(save_state['die'])
	if loaded_die == null:
		print("Could not load die during die_prop_pair loader")
		return null
	var loaded_props : RollProperties = RollProperties.load_from_save_dict(save_state['roll_properties'])
