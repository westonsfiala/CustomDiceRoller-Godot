...
func expected_result(properties : RollProperties) -> float:
	...
	var faces : Array = get_faces()
	for face : int in faces:
		var value : float = face
		
		# If value is above min, set to min.
		var minimum_die_roll_value : int = properties.get_minimum()
		...
		var maximum_die_roll_value : int = properties.get_maximum()
		...
		var explode : bool = properties.get_explode()
		if(explode and value == maximum()):
			var num_sides : int = faces.size()
			if(num_sides == 1):
				value = maximum() + average()
			else:
				value = maximum() + float(num_sides)/(num_sides-1) * average()
		
		var count_below : int = properties.get_count_below()
		var count_above : int = properties.get_count_above()
